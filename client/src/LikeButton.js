...
export default class LikeButton extends Component {
  ...
  like = (data) => {
    this.toggleLike();
    fetch(
      'https://cors-anywhere.herokuapp.com/https://firstdibs.herokuapp.com/browse/favs',
      {
        method: 'POST',
        headers: {
  ...
  unlike = (id) => {
    this.toggleLike();
    fetch(
      `https://cors-anywhere.herokuapp.com/https://firstdibs.herokuapp.com/browse/favs/${id}`,
      {
        method: 'DELETE',
      },
    );
  ...
  render() {
    return (
      <i
        className={
          this.state.liked ? 'fa fa-heart fa-2x' : 'far fa-heart fa-2x'
        }
